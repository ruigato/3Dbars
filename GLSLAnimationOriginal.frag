...
vec3 animateBarPattern(vec2 uv, vec4 pos) {
    ...
    float bar_id = pos.z;      // Normalized bar ID in B channel
...
vec3 animateRandomBars(vec2 uv, vec4 pos) {
    ...
    float hashValue = random(vec2(bar_id, timeStep));  // Unique hash for each bar at each time step
...
vec3 sampleTexture(vec4 pos) {
    ...
    if (u_texture_mode == 0) {
        ...
        // Uses normalized bar ID as V coord and position along bar as U coord
        float u = bar_pos;                   // Position along bar
        float v = bar_id;                    // Bar ID
        texCoord = vec2(u, v);
    }
