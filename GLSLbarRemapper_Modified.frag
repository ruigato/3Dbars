...
uniform sampler2D sTD2DInputs[2]; // Texture inputs: 0=default, 1=external input
uniform int uUseInputTex;         // Output mode switch: 0 = shader-generated, 1 = remapped input texture
...
void main() {
    ...
    // Sample from the second input texture using remapped UVs
    vec4 inputTexColor = texture(sTD2DInputs[1], remappedUV);
    // Switch between shader-generated color and remapped input texture
    gl_FragColor = mix(visualizerColor, inputTexColor, float(uUseInputTex));
